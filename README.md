...
| `PORT` | `8092` | |
| `CACHE_DIR` | `$TMPDIR/den-reel-cache` | persist with a volume. Must be **exclusively** den-reel's: any top-level *file* that is not a `<youtube_id>.mp4` is treated as abandoned scratch and deleted after 30 minutes. Subdirectories are left alone — den-reel keeps yt-dlp's player cache in `yt-dlp/` and parks four caches in `state/` across restarts, none of which is a trailer and none of which is ever evicted with them: the eviction pass walks *files* at the top of this directory and steps over subdirectories, so nothing in `state/` is deleted by it or counted against `CACHE_MAX_BYTES`. They are `resolve.json` (titles resolved to YouTube ids, a TMDB round trip each), `direct.json` (the googlevideo URLs those resolved to, a yt-dlp run each at 1.2–3.6 s, holding signed URLs that stop working within hours), `crop.json` (measured letterboxes — a second of cropdetect over up to 64 keyframes apiece, and the only one of the four that never expires, since where a trailer's bars are is a fact about the video), and `index.json` (built progressive indexes, ~45 range requests to Google each; worth keeping only because `direct.json` is kept beside it, as an index maps ranges into Google's file and is useful again only alongside the URLs it was built for). Each holds only what is still good. |
