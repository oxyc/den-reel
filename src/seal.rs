//! Sealed config-in-URL — the den-reel half of den-scout/docs/SEALED-CONFIG.md. The config path
//! segment may carry a config SEALED to the addon's X25519 key (libsodium `crypto_box_seal`) instead of
//! plaintext base64, so the BYOK TMDB (+ optional KinoCheck) key is never plaintext in the URL and never
//! stored server-side — decrypted per request, then dropped.
//!
//! Wire format is byte-identical to the Go addon (den-scout), the Rust addon (den-subtitles), and the
//! browser bundle: the decoded segment is `version_byte(0x01) ‖ crypto_box_seal(config_json)`.
//! Decrypt-only (no rng dep).
...
impl Keyring {
    ...
    /// (sealed URLs disabled; legacy plaintext still works). A malformed CURRENT key is an `Err`.
    pub fn from_env(current: &str, prev: &str) -> Result<Option<Keyring>, String> {
        ...
        for p in prev.split(',') {
            if let Err(e) = add(p) {
                eprintln!("den-reel: skipping a malformed REEL_CONFIG_KEYS_PREV entry: {e}");
            }
        }
...
mod tests {
    ...
    // The SAME fixed libsodium crypto_box_seal vector den-scout and den-subtitles test (PyNaCl
    // SealedBox). The interop GATE: this ciphertext, produced by a real libsodium binding, must open in
    // Rust too — proving den-reel's seal is byte-identical to the other addons and the browser bundle.
